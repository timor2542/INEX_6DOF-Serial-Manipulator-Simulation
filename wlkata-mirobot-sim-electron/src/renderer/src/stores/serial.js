    ...
    actions: {
        ...
        async listCH340Ports() {
            ...
            for (const p of ports) {
                ...
                if (info.usbVendorId === 0x1A86) {
                    list.push({ id: await idOf(p), label: `CH340 (${hex(info.usbVendorId)}:${hex(info.usbProductId)})` })
                }
            }
            return list
        },
            ...
            try {
                ...
                if (portId) {
                    ...
                    for (const p of await navigator.serial.getPorts()) {
                        if (await idOf(p) === portId) { port = p; break }
                    }
                }
                if (!port) {
                ...
                this.connected = true
                this.appendLog({ dir: 'sys', text: `Connected: CH340 (${hex(this.portInfo.usbVendorId)}:${hex(this.portInfo.usbProductId)}) @${baudRate}`, ts: Date.now() })
        ...
        async disconnect() {
            try {
                ...
                if (this.port?.readable) try { await this.port.close() } catch { }
            } catch { }
            this.reader = null
            this.writer = null
            this.port = null
        ...
        async send(data /* Uint8Array */) {
            ...
            try {
                ...
                let preview = ''
                try { preview = new TextDecoder().decode(data) } catch { }
